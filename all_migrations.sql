...
-- 1. Update Foreign Keys to point to public.tasks instead of workspace_tasks
DO $$
BEGIN
    -- task_activity_logs
    ALTER TABLE public.task_activity_logs DROP CONSTRAINT IF EXISTS task_activity_logs_task_id_fkey;
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'workspace_tasks') THEN
        EXECUTE 'DROP TRIGGER IF EXISTS tr_task_audit_notification ON public.workspace_tasks';
    END IF;
END $$;
