...
// Find the very first occurrence of submitRequirementAnalysis
const firstSubmit = content.indexOf('export async function submitRequirementAnalysis');

if (firstSubmit !== -1) {
  content = content.slice(0, firstSubmit);
}
...
const newBlock = `export async function submitRequirementAnalysis(reqId: string, payload: any, performedBy: string, action?: 'ACCEPT' | 'HOLD' | 'CANCEL') {
...
export async function processApprovalAction(reqId: string, action: string, remarks: string, performedBy: string) {
  ...
  if (req.requester_id && req.requester_id !== performedBy) {
    await dispatchNotification(req.requester_id, \`Requirement \${action}\`, \`Requirement \${req.code} ("\${req.title}") has been marked as \${action}.\`, \`/requirements/\${reqId}\`, 'REQUIREMENT', \`STATUS_\${action.toUpperCase()}\`).catch(e => console.error("Failed to notify requester", e));
  }
  revalidatePath(\`/requirements/\${reqId}\`);
  return { success: true };
}
...
console.log('Clean requirements.ts applied');
