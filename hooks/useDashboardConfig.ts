"use client";

import { useState, useEffect, useCallback } from "react";

export interface DashboardWidgetConfig {
  id: string; // unique ID for the widget instance
  type: string; // widget type (e.g., 'kpi', 'chart')
  colSpan: 1 | 2 | 3 | 4; // grid column span
  rowSpan: number; // grid row span
  order: number; // display order
  props?: Record<string, any>; // custom props for the widget
}

export const DEFAULT_DASHBOARD_LAYOUT: DashboardWidgetConfig[] = [
  { id: "kpi_main", type: "kpi", colSpan: 4, rowSpan: 1, order: 1 },
  { id: "upcoming_deadlines", type: "upcoming_deadlines", colSpan: 4, rowSpan: 2, order: 2 },
  { id: "recent_tickets", type: "recent_tickets", colSpan: 2, rowSpan: 2, order: 3 },
  { id: "activity_feed", type: "activity_feed", colSpan: 2, rowSpan: 2, order: 4 },
  { id: "risk_intel", type: "risk_intelligence", colSpan: 2, rowSpan: 2, order: 5 },
  { id: "status_comp", type: "status_comparison", colSpan: 2, rowSpan: 2, order: 6 },
  { id: "reso_velo", type: "resolution_velocity", colSpan: 4, rowSpan: 2, order: 7 },
  { id: "health_sla", type: "sla_governance", colSpan: 2, rowSpan: 2, order: 8 },
  { id: "health_workload", type: "workload_intelligence", colSpan: 2, rowSpan: 2, order: 9 },
  { id: "charts_row", type: "charts", colSpan: 4, rowSpan: 2, order: 10 },
  { id: "kanban_board", type: "kanban", colSpan: 4, rowSpan: 2, order: 11 },
  { id: "team_performance", type: "team_performance", colSpan: 4, rowSpan: 2, order: 12 },
];

export function useDashboardConfig(dashboardCode: string = 'DEFAULT_COMMAND_CENTER') {
  const [layout, setLayout] = useState<DashboardWidgetConfig[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchLayout = useCallback(() => {
    setLoading(true);
    // Bust cache to force the new layout updates for all users
    const storageKey = `dashboard_layout_v2_${dashboardCode}`;
    const saved = localStorage.getItem(storageKey);

    if (saved) {
      try {
        const parsedLayout: DashboardWidgetConfig[] = JSON.parse(saved);
        // We could merge missing default widgets here if necessary
        setLayout(parsedLayout.sort((a, b) => a.order - b.order));
      } catch (e) {
        console.error("Failed to parse dashboard layout, resetting to default", e);
        setLayout(DEFAULT_DASHBOARD_LAYOUT);
      }
    } else {
      setLayout(DEFAULT_DASHBOARD_LAYOUT);
    }
    setLoading(false);
  }, [dashboardCode]);

  useEffect(() => {
    fetchLayout();
  }, [fetchLayout]);

  const saveLayout = useCallback((newLayout: DashboardWidgetConfig[]) => {
    setLayout(newLayout);
    const storageKey = `dashboard_layout_v2_${dashboardCode}`;
    localStorage.setItem(storageKey, JSON.stringify(newLayout));
  }, [dashboardCode]);

  const resetToDefault = useCallback(() => {
    setLayout(DEFAULT_DASHBOARD_LAYOUT);
    const storageKey = `dashboard_layout_v2_${dashboardCode}`;
    localStorage.removeItem(storageKey);
  }, [dashboardCode]);

  return {
    layout,
    loading,
    saveLayout,
    resetToDefault
  };
}
