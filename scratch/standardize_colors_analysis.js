...
const file = 'app/requirements/[id]/page.tsx';

let content = fs.readFileSync(file, 'utf8');

// 1. Replace all header backgrounds
content = content.replace(/bg-gradient-to-r from-[a-z]+-500\/15 via-[a-z0-9]+\/[0-9]+ to-[a-z0-9]+\/[0-9]+ dark:from-[a-z]+-600\/30 dark:via-[a-z0-9]+\/[0-9]+ dark:to-[a-z0-9]+\/[0-9]+( select-none)?/g, 'bg-surface dark:bg-elevated/50$1');
...
content = content.replace(/className=\"w-4 h-4 text-[a-z]+-[0-9]{3}\"/g, 'className=\"w-4 h-4 text-accent\"');
...
content = content.replace(/text-[a-z]+-600 dark:text-[a-z]+-400/g, 'text-accent');

// 8. Replace individual icon colors in labels
content = content.replace(/className=\"w-3\.5 h-3\.5 text-[a-z]+-500\"/g, 'className=\"w-3.5 h-3.5 text-accent\"');
