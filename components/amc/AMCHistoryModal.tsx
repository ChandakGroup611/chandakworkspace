...
export function AMCHistoryModal({
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-surface/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`relative w-full max-w-2xl max-h-[85vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden ${ "theme-card-structural " } border`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {loading ? (
            ...
            <div className="relative border-l-2 border-border dark:border-border ml-4 space-y-8">
              {logs.map((log) => (
                <div key={log.id} className="relative pl-6">
                  ...
                  <div className={`p-4 rounded-xl theme-card-structural shadow-sm`}>
                    ...
                    <div className="flex items-center justify-between mb-3 border-b border-border/20 pb-3">
                      ...
                      <span className="text-xs text-muted font-medium bg-gray-500/10 px-2 py-1 rounded">
                    ...
                    {log.operation === 'UPDATE' && log.before_values && log.after_values && (
                      <div className="mt-4 space-y-3">
                        <p className="text-xs font-semibold text-muted uppercase tracking-wider">Changes Detected</p>
                    ...
                    {log.operation === 'INSERT' && (
                       <p className="text-sm text-muted">Record created with initial values.</p>
                    )}
