...
function SortableColumnItem({ item, onToggle }: { item: UserReportLayout, onToggle: (id: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center gap-3 p-3 rounded-xl transition-all ${item.is_visible ? "theme-card-structural border-border shadow-sm dark:bg-slate-800 dark:border-slate-700" : "bg-surface border-dashed border-slate-300 opacity-60 dark:bg-slate-900 dark:border-border"}`}>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <AppButton variant="secondary"
          ...
          className={`p-2 rounded-lg border transition-colors ${item.is_visible ? "bg-emerald-50 border-emerald-200 text-success hover:bg-emerald-100 dark:bg-success/10 dark:border-emerald-500/20 dark:text-success" : "bg-slate-100 border-border text-slate-400 hover:bg-slate-200 dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700"}`}
...
export default function DynamicReportBuilder({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
      ...
      <div className="relative theme-card-structural dark:bg-slate-900 rounded-2xl  w-full max-w-4xl max-h-[90vh] flex flex-col border-border dark:border-border animate-in fade-in zoom-in-95">
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-surface/50 dark:bg-slate-950/30">
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-2">
              ...
              {draftLayout.length === 0 && (
                <div className="text-center py-10 text-sm text-muted italic">No columns selected.</div>
              )}
            </div>
          </div>
          ...
          <div className="w-full md:w-72 lg:w-80 flex flex-col min-h-0 bg-surface dark:bg-slate-900">
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-2">
              {unselectedFields.map(field => (
                <div 
                  key={field.field_key}
                  className="flex items-center justify-between gap-2 p-2.5 rounded-lg border border-slate-100 dark:border-border hover:border-theme-btn-primary/30 dark:hover:border-theme-btn-primary/30 transition-colors group cursor-pointer"
