...
export function ExecutiveKPIWidget({ analytics, kpis: globalKpis, onDrillDown }: ExecutiveKPIWidgetProps) {
  ...
  return (
    <BaseWidget
      ...
      badge={
        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-theme-btn-primary/10 text-theme-icon border border-theme-btn-primary/20 shrink-0 inline-flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-theme-btn-primary animate-pulse" />
          <AnimatedCounter value={totalCount} duration={600} /> Total Items
        </span>
      }
      collapsible={true}
    >
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5 w-full">
        ...
        >
          <div className="min-h-[120px] h-full overflow-hidden relative rounded-xl bg-surface/60 border border-border/70 hover:border-emerald-500/50 hover:bg-surface transition-all shadow-xs p-3.5 flex flex-col justify-between kpi-tile-interactive" style={{ "--kpi-glow-color": "rgba(16, 185, 129, 0.25)" } as React.CSSProperties}>
            <div className="absolute top-0 left-0 w-full h-[3px] bg-success shadow-[0_0_12px_rgba(16,185,129,0.7)]" />
            <div className="absolute -bottom-3 -right-3 p-2 opacity-[0.05] dark:opacity-[0.08] kpi-watermark-icon text-success">
            ...
            <div className="flex justify-between items-center mb-1 relative z-10">
              ...
              <div className="p-1 rounded-md bg-success/10 text-success group-hover:scale-110 transition-transform duration-200">
                <FolderOpen className="h-3.5 w-3.5" />
              </div>
            </div>
            
            <div className="my-1 relative z-10">
              <span className="text-2xl sm:text-3xl font-black text-foreground drop-shadow-sm group-hover:text-emerald-500 transition-colors">
                <AnimatedCounter value={kpis.workspaces?.total || 0} />
              </span>
            </div>

            <div className="flex items-center gap-1 text-[10px] text-muted-foreground relative z-10">
              <span className="text-emerald-500 font-semibold"><AnimatedCounter value={kpis.workspaces?.resolved || 0} /> Done</span>
              <span>·</span>
              <span><AnimatedCounter value={(kpis.workspaces?.total || 0) - (kpis.workspaces?.resolved || 0)} /> Open</span>
            </div>
          </div>
        </div>
        ...
        <div 
          ...
          className="block group cursor-pointer"
        >
          <div className="min-h-[120px] h-full overflow-hidden relative rounded-xl bg-surface/60 border border-border/70 hover:border-teal-500/50 hover:bg-surface transition-all shadow-xs p-3.5 flex flex-col justify-between kpi-tile-interactive" style={{ "--kpi-glow-color": "rgba(20, 184, 166, 0.25)" } as React.CSSProperties}>
            <div className="absolute top-0 left-0 w-full h-[3px] bg-teal-500 shadow-[0_0_12px_rgba(20,184,166,0.7)]" />
            <div className="absolute -bottom-3 -right-3 p-2 opacity-[0.05] dark:opacity-[0.08] kpi-watermark-icon text-teal-500">
            ...
            <div className="flex justify-between items-center mb-1 relative z-10">
              ...
              <div className="p-1 rounded-md bg-teal-500/10 text-teal-500 group-hover:scale-110 transition-transform duration-200">
                <Layers className="h-3.5 w-3.5" />
              </div>
            </div>
            
            <div className="my-1 relative z-10">
              <span className="text-2xl sm:text-3xl font-black text-foreground drop-shadow-sm group-hover:text-teal-500 transition-colors">
                <AnimatedCounter value={kpis.sub_workspaces?.total || 0} />
              </span>
            </div>

            <div className="flex items-center gap-1 text-[10px] text-muted-foreground relative z-10">
              <span className="text-teal-500 font-semibold"><AnimatedCounter value={kpis.sub_workspaces?.resolved || 0} /> Done</span>
              <span>·</span>
              <span><AnimatedCounter value={(kpis.sub_workspaces?.total || 0) - (kpis.sub_workspaces?.resolved || 0)} /> Open</span>
            </div>
          </div>
        </div>
        ...
        <div 
          ...
          className="block group cursor-pointer"
        >
          <div className="min-h-[120px] h-full overflow-hidden relative rounded-xl bg-surface/60 border border-border/70 hover:border-blue-500/50 hover:bg-surface transition-all shadow-xs p-3.5 flex flex-col justify-between kpi-tile-interactive" style={{ "--kpi-glow-color": "rgba(59, 130, 246, 0.25)" } as React.CSSProperties}>
            <div className="absolute top-0 left-0 w-full h-[3px] bg-blue-500 shadow-[0_0_12px_rgba(59,130,246,0.7)]" />
            <div className="absolute -bottom-3 -right-3 p-2 opacity-[0.05] dark:opacity-[0.08] kpi-watermark-icon text-blue-500">
            ...
            <div className="flex justify-between items-center mb-1 relative z-10">
              ...
              <div className="p-1 rounded-md bg-blue-500/10 text-blue-500 group-hover:scale-110 transition-transform duration-200">
                <Target className="h-3.5 w-3.5" />
              </div>
            </div>
            
            <div className="my-1 relative z-10">
              <span className="text-2xl sm:text-3xl font-black text-foreground drop-shadow-sm group-hover:text-blue-500 transition-colors">
                <AnimatedCounter value={kpis.tasks?.total || 0} />
              </span>
            </div>
            
            <div className="flex flex-wrap items-center gap-1 text-[10px] font-medium text-muted-foreground relative z-10">
              <span className="text-blue-500 flex items-center gap-0.5 whitespace-nowrap"><CheckCircle className="h-3 w-3 shrink-0" /> <AnimatedCounter value={kpis.tasks?.resolved || 0} /> Done</span>
              <span className="text-muted-foreground/50">·</span>
              <span className="whitespace-nowrap"><AnimatedCounter value={(kpis.tasks?.total || 0) - (kpis.tasks?.resolved || 0)} /> Open</span>
            </div>
          </div>
        </div>
