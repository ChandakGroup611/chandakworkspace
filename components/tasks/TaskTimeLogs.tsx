...
export default function TaskTimeLogs({ taskId }: { taskId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`p-4 rounded-xl border ${"bg-surface border-border"}`}>
        <div className="flex justify-between items-end mb-2">
          <div className="space-y-1">
            ...
            <p className={`text-2xl font-bold ${isLightMode ? "text-slate-900" : "text-white"}`}>{totalLogged.toFixed(1)}h</p>
          </div>
      ...
      {/* Log Form */}
      <form onSubmit={handleSubmit} className={`p-4 rounded-xl border ${isLightMode ? "bg-gray-50 border-gray-200" : "bg-black/20 border-white/5"}`}>
      ...
      <div className="space-y-3">
        ...
        {logs.length === 0 ? (
          ...
          logs.slice().reverse().map(log => (
            <div key={log.id} className={`p-3 rounded-lg border text-sm flex gap-3 ${isLightMode ? "border-gray-100 bg-white" : "border-white/5 bg-white/5"}`}>
              <div className="shrink-0 font-bold text-emerald-600 dark:text-emerald-400 w-12">{log.hours}h</div>
              <div className="flex-1 min-w-0">
                <p className={`truncate ${isLightMode ? "text-gray-800" : "text-gray-200"}`}>{log.description}</p>
